...
class FilmsDescriptionController: UITableViewController {

    private var elements = [DescriptionElement]()

    func setObjectId(_ id: Int16) {
        DataRepository.shared.fetchFilm(with: id) { [weak self] film in
            self?.elements = DescriptionBuilder().build(from: film)
